...
async function isPointOnCurve(x, y) {
    const a = 0n;
    const b = 7n;
    return (y * y) % p === ((x * x * x) + (a * x) + b) % p;
}
async function modInverse(k, n) {
    const egcd = async (a, b) => {
        if (a === 0n)
            return [b, 0n, 1n];
    ...
    if (g !== 1n)
        return 0n; // No modular inverse exists
    return ((x % n) + n) % n;
}
async function ellipticCurveMultiply(k, x, y, p) {
    // Handle special cases: return point at infinity
    if (k === 0n || (x === 0n && y === 0n)) {
        return [0n, 0n];
    }
    let result = [0n, 0n]; // Initialize result as point at infinity
    let current = [x, y]; // Start with the input point
...
async function ellipticCurveAdd(x1, y1, x2, y2, p) {
...
async function modularTests(point1, point2, G_x, G_y, p) {
    //Associvity:
    ...
    //Identity Element
    const O = [0n, 0n]; // Point at infinity
    ...
    //Inverse element.
    const negP = [point2[0], (-point2[1] + p) % p];
...
export async function verifyECDSA2(message, s, r, pub_x, pub_y) {
    const p = BigInt('0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEFFFFFC2F');
...
export async function publicKeyToCompressed(x, y) {
...
export async function proveableECDSAreturnR(ee, s, r, pub_x, pub_y) {
    const p = BigInt('0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEFFFFFC2F');
    const n = BigInt('0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEBAAEDCE6AF48A03BBFD25E8CD0364141');
    const G_x = BigInt('55066263022277343669578718895168534326250603453777594175500187360389116729240');
    const G_y = BigInt('32670510020758816978083085130507043184471273380659243275938904335757337482424');
...
//# sourceMappingURL=proveableECDSA.js.map